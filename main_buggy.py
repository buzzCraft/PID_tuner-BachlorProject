...
class step_analytics:
    ...
    def plot(self, show=True):
        ...
        self.normal_plot = plt
        if show:
            plt.show()
    def plot_detailed(self, show = True):
        print("Plotting")
        #Plotting the step
        ax = sns.lineplot(data=self.step_df[self.measured_value], color="r")
        ...
        _ = ax.hlines(self.prosent63[1], xmin=0, xmax =(self.prosent63[1]-5)*self._sampling_time, color="grey", linestyles='dashed' )
        _ = ax.vlines((self.prosent63[1]-5) * self._sampling_time, ymin=self.start[1], ymax=self.prosent63[1], color="grey",
                      linestyles='dashed')
    ...
    def find_peak(self):
        ...
        if self.positive_step:
            ...
            self.step_df['peak'] = self.step_df[self.measured_value][(self.step_df[self.measured_value].shift(1) > self.step_df[self.measured_value]) &(self.step_df[self.measured_value].shift(-1) > self.step_df[self.measured_value])]
        print("Mean print")
        print((self.step_df['peak'].max()+self.step_df['peak'].min())/2)

    def find_delta(self, peak_val):
        self.dY = abs((peak_val*(1-self._factor) - self.start[1])*(1-self._factor))
    ...
    def find_tau(self, resp=False):
        if not resp:
            self.tau = 1.5 * ((abs(self.prosent63[0])*self._sampling_time)-(abs(self.prosent28[0])*self._sampling_time))
        else:
        # Old implementation

            self.tau = self.step_df.iloc[self.prosent63[0]]['Time'] - self.step_df.iloc[resp]['Time']

    def find_theta(self, resp=False):
        if not resp:
            time = self.tau/self._sampling_time
            self.theta = ((abs(self.prosent63[0])*self._sampling_time)-self.tau) - ((abs(self.start[0]) * self._sampling_time))
            # Har theta = (tid_63% - tau) - start_step
    ...
    def find_28_63(self):
        ...
        if self.positive_step:
            ...
            self.twenty8 = self.dY * 0.283 + self.start[1]
            # Find time and value for 28% of step
            for i in range(1, len(self.step_df)):
                if self.twenty8 <= self.step_df.iloc[-i][self.measured_value]:
                    self.prosent28.append(-i)
                    self.prosent28.append(self.step_df.iloc[-i][self.measured_value])
            # Find time and value for 63% of step
            for i in range(1, len(self.step_df)):
            ...
            self.six = -self.dY * 0.63 + self.start[1]
            self.twenty8 = -self.dY * 0.28 + self.start[1]
            # Find time and value for 28% of step
            for i in range(1, len(self.step_df)):
                if self.twenty8 >= self.step_df.iloc[-i][self.measured_value]:
                    self.prosent28.append(-i)
    ...
    def find_parameters(self):
        ...
        self.find_delta(peak_val)
        #Test for calculating factor based on dU
        self.find_factor()
